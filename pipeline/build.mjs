...
// Constanța: ONE CT BUS feed split by route_type — buses (route_type 3) on the
// road graph. There is no metro here, so the engine's metro
...
// fully accented from SMTT ("Dâmbovița (Ion Barac)").
...
// Line keys are bare numbers with optional letters on either end: plain 26,
// night lines suffixed N (26N, 301Na), express lines prefixed E (E2, E6),
// tram variants suffixed L (2L, 7L), suburban letters (36L, 401A). Sorting
// splits each key into prefix, number and suffix so 9 < 10 holds INSIDE every
// family, and the families themselves stay grouped: plain numbers, then E…
const keyParts = (s) => {
  const m = /^(\D*)(\d*)(.*)$/.exec(s);
...
// SMTT numbers all three modes from ONE pool with no number used twice, so a
// key IS the number printed on the vehicle and nothing has to be stripped for
// display. (Sofia needs ТБ/ТМ prefixes here; Timișoara does not, and the
// identity keeps the sibling code paths aligned.)
const dispLine = (l) => l;
const dispList = (s) => s;
...
// Weld dangling endpoints (nodes used by ONE way only) to the nearest vertex
// of another way of the same kind within `maxGap` m, as synthetic two-node
// ways. Service tracks stay excluded — this only reconnects running lines that
// OSM left as islands. In Belgrade it is the TRAM network that needs it: the
// Blok 45 terminal loop touches the Novi Beograd corridor geographically but
// shares no node with it, and a 2 m hole is enough to strand the terminus of
// four lines (their last stops then drop out as "500 m from every line").
function weldRailGaps(elements, kindWanted = 'subway', maxGap = 60) {
  const SERVICE_X = new Set(['yard', 'siding', 'spur', 'crossover']);
  const ways = elements.filter((e) => e.type === 'way' && railKind(e.tags) === kindWanted
    ...
    for (const end of [0, w.nodes.length - 1]) {
      ...
      for (const o of ways) {
        ...
        for (let i = 0; i < o.nodes.length; i++) {
          ...
          if (d < maxGap && (!best || d < best.d)) best = { d, nid: o.nodes[i], g };
        }
      }
      if (best) {
        elements.push({
          type: 'way', id: synId--, nodes: [nid, best.nid],
          geometry: [p, best.g], tags: { railway: kindWanted },
        });
        nodeUse.set(nid, 2);
...
// Line numbers are unique across the whole network (see dispLine above), so a
// key is simply the number as printed. Uppercased for the suffix letters,
// which the feed writes inconsistently ("301Na" alongside "401A").
const norm = (sn) => sn.trim();

// ONE feed, ONE cfg: routeTypes splits the CT BUS bundle per mode
// (3 = bus;
...
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/constanta.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['5-40']),
  feeds: [
    {
      tag: 'constanta', dir: 'data/gtfs', routeTypes: ['3'],
      mapKey: (sn) => norm(sn),
    },
  ],
}];
...
// extract and the named OSM objects around the city (data/osm/constanta-names.json,
...
const nameDict = (() => {
  const docs = [];
  for (const f of ['data/osm/constanta.json', 'data/osm/constanta-names.json']) {
...
// Belgrade needs 300: its suburban lines detour deep into villages, and a scan
// of every excursion in the built network showed the split cleanly — of the 26
// excursions longer than 600 m out-and-back, ALL 26 serve a stop, while the
// longest excursion serving nothing is 527 m. So 2 x 300 keeps every real
// service detour and still reaches the artifacts.
const SPUR_MAX = 300;
...
async function processMode(cfg) {
  ...
  // an all-metro mixed set (the shared M1+M2+M4 central tunnel) must
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      // the whole route row is passed on: one cfg holds two route_types here,
    ...
    // The CGM feed ships a direction_id COLUMN but leaves it empty on every
    // one of its 31 965 trips — the return direction exists as separate trips
    // with a mirrored headsign (the Athens/OSY defect, here feed-wide: 116 of
    // 122 lines lost their return leg, metro M1–M4 included). Grouped by
    // direction_id alone both directions collapse into one bucket and only
    // the busier one gets drawn. Pre-scan direction diversity per line; where
    ...
    for (const s of await readCsv(join(fdir, 'stops.txt'))) {
      ...
      // CT BUS publishes its stop names properly cased and accented, so
    ...
    // trim the shape to the passenger stretch: CGM shapes overshoot the
    ...
    // Each anchor is searched only in ITS OWN HALF of the shape. A globally
    // nearest match breaks circular routes, where the first and last stop are
    // the same kerb: Belgrade's line 67 (Blok 70A → Zeleni venac → Blok 70A)
    // matched both anchors against the end of its own loop and was trimmed
    // from 14.2 km to 39 m. A depot tail is short by nature, so the true
    // anchors always sit in their own half.
    for (const r of feedReps) {
      if (r.pseudo || r.stopSeq.length < 2) continue;
      const near = (st, lo0, hi0) => {
        let bi = lo0, bd = Infinity;
  ...
  // railKind: a way tagged railway=construction counts as the kind it is being
  // built as, so a reopened stretch OSM has not retagged yet still carries its line
  if (cfg.railKeep) osm.elements = osm.elements.filter((e) => cfg.railKeep.has(railKind(e.tags)));
  // OSM maps per-direction tracks that meet nowhere: at junctions and terminal
  // loops the ways pass within metres but share NO node, which strands whole
  // branches on separate islands. Every dangling endpoint gets welded to the
  // nearest other-way vertex within 60 m so Viterbi can route through. In
  // Belgrade this is the tram network's problem — the Blok 45 loop sits 2 m
  // from the corridor that feeds it, and without the weld the terminus of
  // lines 7L, 9L, 11 and 13 is unreachable.
  if (cfg.railKeep?.has('tram')) {
    const n = weldRailGaps(osm.elements, 'tram');
    if (n) log(`welded ${n} dangling tram endpoints to nearby tracks`);
  }
  ...
  for (const r of reps) {
    ...
    if (r.pseudo) {
      ...
      opts = { gapMin: GAP_MIN };
      // Rail: cap how many candidate slots ONE OSM way may claim. Tram and
      // metro tracks stack vertically (a tunnel under the surface tracks it
      // shadows), so without the cap every slot at such a point can belong to
      // the same way and the connected alternative never enters Viterbi —
      // which tears the line apart (diagnosed in Vienna at Schottentor).
    ...
    if (spurs.length) {
      ...
      // terminal repair inserted: 24 of Timișoara's 45 trims dropped nothing
      // that way and left their strokes on the map — M27 kept five arcs of a
      // roundabout its route passes 164 m away. Distance to the FINAL path is
      // immune to that: a segment the line still travels sits at 0 m from it,
      // so this can never break a corridor open.
      const FAR = 40;
      const far = (si) => {
        ...
        for (let i = 0; i + 1 < trimmed.length; i++) {
          const [x1, y1] = trimmed[i], [x2, y2] = trimmed[i + 1];
          const dx = x2 - x1, dy = y2 - y1, L = dx * dx + dy * dy;
          const t = L === 0 ? 0 : Math.max(0, Math.min(1, ((mx - x1) * dx + (my - y1) * dy) / L));
          const d = Math.hypot(mx - (x1 + t * dx), my - (y1 + t * dy));
          if (d < best) best = d;
          if (best <= FAR) return false;
        }
        return best > FAR;
      };
      let swept = 0;
      for (const si of [...res.usedSegs]) {
        if (far(si)) { res.usedSegs.delete(si); res.usedIv.delete(si); swept++; }
      }
  ...
  for (const e of stopAgg.values()) {
    ...
    // that neither the CGM shape nor OSM knows anymore). A disc stranded in
...
// `nolabel` (its stroke stays, the track takes over its numbers).
{
  ...
  for (const g of groups.values()) {
    ...
    // beside it, and the tram numbers above them keep the rail colour, so the
    // split must run on busLines there. Without this a track row is stuck at
    // two colours and a trolleybus riding a tram street comes out navy (user
    // report, Naples 254; here it is 15/16 sharing the tracks of trams 1/9).
    ...
    if (navy && TSET.size) {
      ...
      if (tl.length && tl.length < navy.length) {
        ...
        baseProps.ntLines = dispList(navy.filter((l) => !TSET.has(l)).join(', '));
      }
    }
...
const nameFeatures = mergeRuns(streetFeatures
  ...
  .map((r) => {
    // Single-line labels: Romanian is already Latin, so there is no second
    // reading to add (the Greek, Bulgarian and Serbian siblings carry one).
    return {
      type: 'Feature',
      geometry: { type: 'LineString', coordinates: r.coords },

...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: CT BUS GTFS (gtfs.ro)' }));
  ...
  document.getElementById('count').textContent = `(${nBus} bus lines)`;
  ...
  // A track row can carry THREE colours, not two: the tram numbers in the rail
  // colour, then the numbers adopted from the roadway under them — and inside
  // that adopted half the trolleybuses keep their green, exactly as they do on
  // a roadway row (user 14.08.2026: every number in the colour of its own
  // category, and a street can carry more than two categories).
  ...
  // The base tiles carry a Latin reading of their own for ~40% of the Cyrillic
  // street names, and the OpenFreeMap style prints it BEFORE the local name on a
  // single line ("Graf Ignatiev Граф Игнатиев"). Our labels put the local name first with the
  ...
  // used only where the whole name is Cyrillic — a name that is already part
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: CT BUS GTFS (gtfs.ro)';
      ...
      a.download = `constanta-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `constanta-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the metropolitan lines reach well past the city,
  // so fitting the data bbox would open on countryside with the city as a
  // small core. Open on the centre instead.
  map.jumpTo({ center: [28.635, 44.18], zoom: 13.0 });
}
